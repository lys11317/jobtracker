...
$versionedZipPath = Join-Path $root "outputs\job-tracker-edge-extension-v1.4.7.zip"
...
if ($failures.Count -eq 0) {
    ...
    Assert-Contains $readme "1.4.7" "README version"
...
if (-not (Test-Path -LiteralPath $versionedZipPath)) {
    Add-Failure "Missing versioned zip: outputs\job-tracker-edge-extension-v1.4.7.zip"
}
